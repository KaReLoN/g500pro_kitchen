...
#		}
		...
		open (RAMDISKFILE, ">$ARGV[0]-ramdisk.cpio.lz");
		binmode(RAMDISKFILE);
		print RAMDISKFILE $ram1 or die;
		close RAMDISKFILE;

		print "Ramdisk written to '$ARGV[0]-ramdisk.cpio.lz'\n";
		...
		foreach my $tool ("lzop", "cpio") {
			die colored ("\nError: $tool binary not found!", 'red') . "\n"
				if system ("command -v $tool >/dev/null 2>&1");
		}
		print "Ramdisk size: ";
		system ("lzop -d -c ../$ARGV[0]-ramdisk.cpio.lz | cpio -i");
