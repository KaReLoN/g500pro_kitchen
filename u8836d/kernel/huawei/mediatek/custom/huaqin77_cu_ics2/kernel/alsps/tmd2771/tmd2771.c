...
int force_report_proximity = 0;
module_param(force_report_proximity,int,00664);

#ifdef DEBUG
int enable_debug_messages = 1;
module_param(enable_debug_messages,int,00664);
#endif

/*----------------------------------------------------------------------------*/

#ifdef DEBUG
#define APS_IF_DEBUG_ENABLED(X) \
    if(enable_debug_messages){X;}
#else
#define APS_IF_DEBUG_ENABLED(X) \
    do { } while(0)
#endif
...
#define APS_LOG(fmt, args...)    APS_IF_DEBUG_ENABLED(printk(KERN_INFO APS_TAG fmt, ##args))
#define APS_DBG(fmt, args...)    APS_IF_DEBUG_ENABLED(printk(KERN_INFO APS_TAG fmt, ##args))
/******************************************************************************
 * extern functions
*******************************************************************************/
...
static int tmd2771_check_and_clear_intr(struct i2c_client *client) 
{
	...
	APS_DBG("yucong tmd2771_check_and_clear_intr status=0x%x\n", buffer[0]);
	res = 1;
	intp = 0;
	intl = 0;
	if(0 != (buffer[0] & 0x20))
...
//	struct tmd2771_priv *obj = i2c_get_clientdata(client);
	...
	APS_DBG("tmd2771_check_and_clear_intr status=0x%x\n", buffer[0]);
	res = 1;
	intp = 0;
	intl = 0;
	if(0 != (buffer[0] & 0x20))
...
//get adc channel 0 value
	...
	APS_DBG("yucong: TMD2771_CMM_C0DATA_L = 0x%x\n", als_value_low[0]);
	...
	APS_DBG("yucong: TMD2771_CMM_C0DATA_H = 0x%x\n", als_value_high[0]);
...
//get adc channel 1 value
	...
	APS_DBG("yucong: TMD2771_CMM_C1DATA_L = 0x%x\n", als_value_low[0]);	
	...
	APS_DBG("yucong: TMD2771_CMM_C1DATA_H = 0x%x\n", als_value_high[0]);	
...
static int tmd2771_get_ps_value(struct tmd2771_priv *obj, u16 ps)
{
	...
	APS_LOG("als_value = %05d\n",obj->als);

	if(force_report_proximity >= 6){
		APS_ERR("force_report_proximity:%d must return 1\n",force_report_proximity);
		return 0;
	}
	...
	{
		//invalid = 1;
		APS_DBG("light too high will result to failt proximity\n");
		return 1;  /*far away*/
	}
...
#if 1
		if(intr_flag_value){
				APS_DBG("yucong interrupt value ps will < 750");
				databuf[0] = TMD2771_CMM_INT_LOW_THD_LOW;	
				databuf[1] = (u8)((atomic_read(&obj->ps_thd_val_low)) & 0x00FF);
				...
				APS_DBG("yucong interrupt value ps will > 900");
				databuf[0] = TMD2771_CMM_INT_LOW_THD_LOW;	
