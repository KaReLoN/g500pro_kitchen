...
/* backlight pwm frequency = 25.3906k Hz */
unsigned int bl_div = CLK_DIV8;	
unsigned int bl_frequency = 52000000;
#else
...
static int brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	else if (pmic_type == MT65XX_LED_PMIC_BUTTON) 
	{
		if (level) 
		{
			//kaka_12_0419 for Keypad LED brightness change
			//First parameter is Duty for 0-31
			unsigned int duty = 3; //A52 want this brightness
			//kaka_12_0419 end
			hwBacklightKPLEDTuning(duty, 0x0);//kaka_12_0419  //duty = 0x09
	...
	switch (cust->mode) {
		case MT65XX_LED_MODE_PWM:
			if(strcmp(cust->name,"lcd-backlight") == 0)
			{
				...
				#ifdef HQ_LCD_ESD_CHECK
				printk("havesuspended:%d\n",havesuspended);
				if(havesuspended == TRUE)
				    return 1;
				#endif
...
#ifdef HQ_LCD_ESD_CHECK
int mt65xx_led_set_cust_public(struct cust_mt65xx_led *cust, int level)
{
	return mt65xx_led_set_cust(cust,level);
}
#endif
...
#elif defined (CONFIG_ARCH_MT6575)  || defined (CONFIG_ARCH_MT6577)
			//duty:0-16
			if((level >= 0) && (level <= 15))
			{
		...
		switch (g_leds_data[i]->cust.mode) {
		    case MT65XX_LED_MODE_PWM:
			    if(strcmp(g_leds_data[i]->cust.name,"lcd-backlight") == 0)
			    {
					...
					//hupeng 120713, turn off lcd backlight by gpio in recovery mode to fix the screen flash issue
					#ifdef HQ_PROJECT_A61P_HUAWEI
					brightness_set_gpio(LCD_BACKLIGHT_GPIO,0);
					#endif
