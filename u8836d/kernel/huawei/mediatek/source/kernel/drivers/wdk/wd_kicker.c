...
//				if(cpu == 0)//Fix me, hotplug callback function run cpu 0, so can not operate cpu1 local wdt
			...
			kick_bit = local_bit;
			spin_unlock(&lock);
			dbgmsg("[WDK]: cpu:%d, kick local wdt\n", cpu);
			// kick local wdt
//			if(cpu == 0)//Fix me, hotplug callback function run cpu 0, so can not operate cpu1 local wdt
		...
		dbgmsg("[cpu-%d:thread:%d] %d-%02d-%02d %02d:%02d:%02d.%u UTC; android time %d-%02d-%02d %02d:%02d:%02d.%03d\n", cpu, current->pid,
	...
	dbgmsg("[WDK] WDT kicker thread stop, cpu:%d, pid:%d\n", cpu, current->pid);
	return 0;
}	
