...
#define VBR_DBG(fmt, args...) \
    if (debug_enable) { printk(fmt, ##args); }
...
static void vibrator_enable(struct timed_output_dev *dev, int value)
{
		...
		while(hrtimer_cancel(&vibe_timer))
                {
                      VBR_DBG("[vibrator]vibrator_enable: try to cancel hrtimer \n");
                }
		...
		{
#if 1
			VBR_DBG("[vibrator]vibrator_enable: vibrator cust timer: %d \n", hw->vib_timer);
		...
		spin_unlock_irqrestore(&vibe_lock, flags);
                VBR_DBG("[vibrator]vibrator_enable: vibrator start: %d \n", value); 
...
{
 		vibe_state = 0;
                VBR_DBG("[vibrator]vibrator_timer_func: vibrator will disable \n");
 		queue_work(vibrator_queue, &vibrator_work);
...
static void vib_shutdown(struct platform_device *pdev)
{
        ...
        VBR_DBG("[vibrator]vib_shutdown: enter!\n");
	if(vibe_state) {
        	VBR_DBG("[vibrator]vib_shutdown: vibrator will disable \n");
		vibe_state = 0;
		vibr_Disable();
	}
}
...
static ssize_t store_vibr_on(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	if(buf != NULL && size != 0)
	{
		VBR_DBG("[vibrator]buf is %s and size is %d \n",buf,size);
		if(buf[0]== '0')
		{
			vibr_Disable();
		}else
