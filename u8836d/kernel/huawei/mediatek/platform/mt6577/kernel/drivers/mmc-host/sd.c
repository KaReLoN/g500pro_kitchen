...
static int msdc_do_request(struct mmc_host*mmc, struct mmc_request*mrq)
{
    ...
    if (!data) {
        ...
        if (dma) {
            ...
            if (!read) {           	
                tmo = jiffies + DAT_TIMEOUT;   //ALPS00331359(For_HUAQIN77_CU_ICS2_ALPS.ICS2.6577.SP.V1_P63)        	
                while (1) {
                    left=msdc_txfifocnt();                    
                    if (left == 0) {
                        break;	
                    }  
                    if (msdc_pio_abort(host, data, tmo)) {  //ALPS00331359(For_HUAQIN77_CU_ICS2_ALPS.ICS2.6577.SP.V1_P63)        	
...
struct mmc_host *emmc_host = NULL; //kaka_12_0224
struct mmc_host *sdcard_host = NULL;//kaka_12_0809
...
static int msdc_drv_probe(struct platform_device *pdev)
{
    ...
    //kaka_12_0224 end
    //kaka_12_0809 add
    else if (1 == pdev->id){ // MSDC1 as T-Flash controller
    	sdcard_host = mmc;
    }
    //kaka_12_0809 end
	host->power_cycle = 0;
