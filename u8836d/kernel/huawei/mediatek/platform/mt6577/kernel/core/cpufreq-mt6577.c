...
/**************************************************
* enable this option to keep maximum frequency when connected to USB
***************************************************/
//#define MTK_CPUFREQ_MAX_FREQ_USB
...
static int cpu_go_suspend = 0;
static int cpu_go_suspend_freq = 0;
...
#define DVFS_F7_MT6577_E1_TM    DVFS_F7_TM
...
{
    static int need_buck_adjust;
    static int group_freq_new;
    static int group_freq_old;
    
#ifdef MTK_BUCK_ADJUST

    switch (freq_new)
    {
#ifndef MTK_CPU_TEST_MODE_FORCE
	case DVFS_F1:
	case DVFS_F2:
	case DVFS_F3:
	case DVFS_F4:
...
/*
    if ( (freq_new == DVFS_F1) || (freq_new == DVFS_F1_TM) ||
	 (freq_new == DVFS_F2) || (freq_new == DVFS_F2_TM) ||
	 (freq_new == DVFS_F3) || (freq_new == DVFS_F3_TM) ||
	 (freq_new == DVFS_F4) || (freq_new == DVFS_F4_TM) )
    {
	group_freq_new = 1;
    }
    else
    if ( (freq_new == DVFS_F5) || (freq_new == DVFS_F5_TM) )
    {
	group_freq_new = 2;
    }
    else
    if ( (freq_new == DVFS_F6) || (freq_new == DVFS_F6_TM) )
    {
	group_freq_new = 3;
    }
    else
    if ( (freq_new == DVFS_F7) || (freq_new == DVFS_F7_TM) )
    {
	group_freq_new = 4;
    }
    else
    ...
    if ( (freq_old == DVFS_F1) || (freq_old == DVFS_F1_TM) ||
	 (freq_old == DVFS_F2) || (freq_old == DVFS_F2_TM) ||
	 (freq_old == DVFS_F3) || (freq_old == DVFS_F3_TM) ||
	 (freq_old == DVFS_F4) || (freq_old == DVFS_F4_TM) )
    {
	group_freq_old = 1;
    }
    else
    if ( (freq_old == DVFS_F5) || (freq_old == DVFS_F5_TM) )
    {
	group_freq_old = 2;
    }
    else
    if ( (freq_old == DVFS_F6) || (freq_old == DVFS_F6_TM) )
    {
	group_freq_old = 3;
    }
    else
    if ( (freq_old == DVFS_F7) || (freq_old == DVFS_F7_TM) )
    {
	group_freq_old = 4;
    }
    else
...
*/    
    ...
    if (group_freq_new == group_freq_old)
    {
	need_buck_adjust = 0;
    }
    else
...
#ifdef MTK_CPUFREQ_MAX_FREQ_USB
...
static int mtk_cpufreq_target(struct cpufreq_policy *policy, unsigned int target_freq, unsigned int relation)
{
    ...
    *******************************/
    ...
    if (likely(get_chip_ver() >= CHIP_6577_E1))
    {
#ifndef MTK_CPU_TEST_MODE_FORCE
    ...
    if (cpu_go_suspend && cpu_go_suspend_freq)
    {
#ifndef MTK_CPU_TEST_MODE_FORCE
	switch (cpu_go_suspend_freq)
	{
	    case 1:
		freqs.new = DVFS_F1;
		break;
	...
	switch (cpu_go_suspend_freq)
	{
		...
		break;
	    case 2:
		freqs.new = DVFS_F2_TM;
		break;
	    case 3:
		freqs.new = DVFS_F3_TM;
		break;
	    case 4:
		freqs.new = DVFS_F4_TM;
		break;
	    case 5:
		freqs.new = DVFS_F5_TM;
		break;
	    case 6:
		freqs.new = DVFS_F6_TM;
		break;
	    case 7:
		freqs.new = DVFS_F7_TM;
		break;
	    default:
		freqs.new = DVFS_F1_TM;
	}
    ...
    else if (mtk_cpufreq_keep_max_freq(freqs.old, freqs.new))
    {
#ifndef MTK_CPU_TEST_MODE_FORCE
        if ((DRV_Reg32(HW_RESV) & (0x1 << 23)) && ((DRV_Reg32(HW_RESV) & (0x1 << 20)) == 0))
            freqs.new = DVFS_F1_TM;
    ...
    if (unlikely(freqs.new > g_limited_freq))
    {
    ...
    if (unlikely(freqs.new < g_limited_min_freq))
    {
    ...
    *************************************************/
    if (unlikely(freqs.old == freqs.new))
    {
...
//            policy->cpuinfo.min_freq = DVFS_F6_MT6577_E1_TM;
	    policy->cpuinfo.min_freq = DVFS_F7_MT6577_E1_TM;
...
**********************************/
void mtk_cpufreq_early_suspend_direct()
{
    if (!g_enable_dvfs)
        return;
...
{
    if (!cpu_go_suspend_freq)
	return;
	
    mtk_cpufreq_early_suspend_direct();
}

/*******************************
* late resume callback function
********************************/
void mtk_cpufreq_late_resume_direct()
{
    if (!g_enable_dvfs)
        return;
...
void mtk_cpufreq_late_resume(struct early_suspend *h)
{
    ...
    mtk_cpufreq_late_resume_direct();
}
...
void cpufreq_thermal_protect(unsigned int limited_opp)
{
    ...
    if (limited_opp)
    {
        ...
        else if (limited_opp == DVFS_OPP7)
        {
#ifndef MTK_CPU_TEST_MODE_FORCE
            if ((DRV_Reg32(HW_RESV) & (0x1 << 23)) && ((DRV_Reg32(HW_RESV) & (0x1 << 20)) == 0))
                g_limited_freq = DVFS_F7_TM;
            else
...
/************************************
* set DVFS status by sysfs interface
*************************************/
...
/***************************
* show current sleep mode freq
****************************/
static ssize_t cpufreq_suspend_freq_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
    int len = 0;
    char *p = buf;

    p += sprintf(p, "Current suspend frequency ID: %d\n", cpu_go_suspend_freq);
...
/************************************
* set DVFS sleep mode freq by sysfs interface
*************************************/
static ssize_t cpufreq_suspend_freq_store(struct kobject *kobj, struct kobj_attribute *attr,const char *buf, size_t n)
{
    int cpu_go_suspend_freq_new = 0;

    if (sscanf(buf, "%d", &cpu_go_suspend_freq_new) == 1)
    {
	if ( (cpu_go_suspend_freq_new >= 0) && (cpu_go_suspend_freq_new < 8) )
	{
	    cpu_go_suspend_freq = cpu_go_suspend_freq_new;
	}
        else
        {
            xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "Bad argument!! Argument should be a digit from 0 to 7\n");
        }
    }
    else
    {
...
cpufreq_attr(cpufreq_suspend_freq);
...
static int cpufreq_pdrv_probe(struct platform_device *pdev)
{
    ...
    *************************************************/
    ...
    ret = sysfs_create_file(power_kobj, &cpufreq_suspend_freq_attr.attr);
...
//            g_limited_min_freq = DVFS_F6_MT6577_E1_TM;
	    g_limited_min_freq = DVFS_F7_MT6577_E1_TM;
        }
        else
        {
